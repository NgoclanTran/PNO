...
##        return (color,shape)
	...
	print "--- WORLD: determine nearest goal ---"
        paths = []
        xMax = self.breadth
	unknownImages = self.getUnknownImagePositions()
        if (len(unknownImages) == 1):
	    self.goal = unknownImages[0]
	    self.hmap = self.getHeuristicMap(self.goal)
            return self.doGreedySearch(pos,unknownImages[0])
        elif (len(unknownImages) == 0):
            return []
        else:
		...
		#print "DETERMINE despos: " + str(paths[i][-1])
