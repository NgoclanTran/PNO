...
# Function BrickPiUG = 1  # Remove to hide errors 

# The I2C ), or -1 (error that could not automatically be resolved, even after
...
class button:
    ...
    def upd(self,I2C_PORT):
      ...
      #For buttons square,triangle,cross,BYTE_TIMEOUT + 1] = (BrickPikPi.SensorI2CIn[I2C_PORT][0][1]>>4)&1
...
def motorRotateDegree(power,deg,port,sampling_time=.1,delay_when_stopping=.05,max_time=5):
    """Rotate the selected motors by specified degre
## DEZE METHODE IS NIET GELDIG BIJ MEERDERE MOTOREN TENZIJ HET AANDRIJVEN VAN TWEE WIELEN TEGELIJK
    Args:
...
       		# Negative number.  This seems to return a 2 byte number.
					BrickPi.Sensor[port] = BrickPi.Sensor[port] - 65535	
				# else:					# Positive Number print str(gyro)
